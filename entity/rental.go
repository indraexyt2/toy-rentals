...
type Rental struct {
	...
	Status             string     `gorm:"size:50;not null;check:status IN ('pending', 'active', 'completed', 'overdue', 'cancelled')" json:"status"`
	...
	LateFee            float64    `gorm:"type:decimal(10,2)" json:"late_fee"`
	DamageFee          float64    `gorm:"type:decimal(10,2)" json:"damage_fee"`
	TotalAmount        float64    `gorm:"-" json:"total_amount"`
	PaymentStatus      string     `gorm:"size:50;not null;default:unpaid;check:payment_status IN ('unpaid', 'pending', 'paid', 'expired', 'failed', 'refunded', 'partially_paid')" json:"payment_status"`
	Notes              string     `gorm:"type:text" json:"notes"`

	User        User         `gorm:"foreignKey:UserID" json:"user"`
	RentalItems []RentalItem `gorm:"foreignKey:RentalID" json:"rental_items"`
	Payments    []Payment    `gorm:"foreignKey:RentalID" json:"payments"`
}
...
func (r *Rental) Validate() []string {
	...
	err := validation.ValidateStruct(r,
		validation.Field(&r.UserID,
			validation.Required.Error("ID pengguna wajib diisi"),
			validation.NotNil.Error("ID pengguna tidak boleh kosong"),
		),
		...
		validation.Field(&r.ActualReturnDate,
			validation.By(validateActualReturnDate),
		...
		validation.Field(&r.PaymentStatus,
			validation.Required.Error("Status pembayaran wajib diisi"),
			validation.In(PaymentStatusUnpaid, PaymentStatusPending, PaymentStatusPaid, PaymentStatusExpired,
				PaymentStatusFailed, PaymentStatusRefunded, PaymentStatusPartiallyPaid).
				Error("Status pembayaran harus salah satu dari: unpaid, pending, paid, expired, failed, refunded, atau partially_paid"),
		),
	)
